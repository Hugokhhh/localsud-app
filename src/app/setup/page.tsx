...
function SetupPage() {
  ...
  if (!valid) {
    return (
      <AuthShell>
        <AuthBrand />
        <div style={{ width: 64, height: 64, margin: '0 auto 16px', borderRadius: 16, background: 'var(--red-soft)', color: 'var(--red)', display: 'grid', placeItems: 'center', fontSize: 24 }}>
  ...
  return (
    <AuthShell>
      <AuthBrand />
      <div style={{ width: 64, height: 64, margin: '0 auto 16px', borderRadius: 16, background: 'var(--ink)', color: 'var(--yellow)', display: 'grid', placeItems: 'center', fontSize: 22 }}>
        ...
        <Field label="Nouveau mot de passe">
          ...
          <div style={{ marginTop: 8 }}>
            <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
              {[0, 1, 2, 3].map(i => (
                <div key={i} style={{ flex: 1, height: 4, borderRadius: 100, background: i < strength ? (strength >= 3 ? 'var(--green)' : 'var(--yellow)') : 'var(--line)' }} />
              ))}
            </div>
